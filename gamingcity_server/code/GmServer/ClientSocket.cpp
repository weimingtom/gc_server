...
bool ClientSocket::Create(const char* pszServerIP, int nServerPort, int nBlockSec, bool bKeepAlive /*= FALSE*/)
{
	// 检查参数 
...
bool ClientSocket::Flush(void)       //? 如果 OUTBUF > SENDBUF 则需要多次SEND（） 
{
	if (m_sockClient == INVALID_SOCKET) {
		return false;
	}
...
void ClientSocket::Destroy(void)
{
	// 关闭 
	struct linger so_linger;
	so_linger.l_onoff = 1;
