...
void PathManager::CreatTroopByData(TroopData& td, Troop& tp)
{
	...
	while(ids != td.szDescrib.end())
	{
		...
		{
			/*TCHAR szinof[256];
			DWORD	nLen = MultiByteToWideChar(CP_UTF8, 0, (*ids).c_str(), (*ids).length(), NULL, 0);
	...
	std::vector<ShapeLine>::iterator isl = td.LineData.begin();
	while(isl != td.LineData.end())
	{
		ShapeLine sl = *isl;
	...
	std::vector<ShapeCircle>::iterator isc = td.CircleData.begin();
	while(isc != td.CircleData.end())
	{
		ShapeCircle sc = *isc;
