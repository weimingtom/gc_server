...
g_land_friend_list = g_land_friend_list or {}
...
function register_friend_pair(player_master,player_farmer_01,player_farmer_02,master_is_win)
	local guid_master = player_master.guid
	local guid_farmer_01 = player_farmer_01.guid
	local guid_farmer_02 = player_farmer_02.guid

	local master_list = player_master.friend_list or g_land_friend_list[guid_master]
	local farmer_01_list = player_farmer_01.friend_list or g_land_friend_list[guid_farmer_01]
	local farmer_02_list = player_farmer_02.friend_list or g_land_friend_list[guid_farmer_02]
	if master_list then
		for k,v in pairs(master_list) do
			clear_table(v)
		end
		clear_table(master_list)
	else
		master_list = {}
		g_land_friend_list[guid_master] = master_list
		player_master.friend_list = master_list
	end
	if farmer_01_list then
		for k,v in pairs(farmer_01_list) do
			clear_table(v)
		end
		clear_table(farmer_01_list)
	else
		farmer_01_list = {}
		g_land_friend_list[guid_farmer_01] = farmer_01_list
		player_farmer_01.friend_list = farmer_01_list
	end
	if farmer_02_list then
		for k,v in pairs(farmer_02_list) do
			clear_table(v)
		end
		clear_table(farmer_02_list)
	else
		farmer_02_list = {}
		g_land_friend_list[guid_farmer_02] = farmer_02_list
		player_farmer_02.friend_list = farmer_02_list
	end

	if master_is_win then
		local table_01 = {guid_master,guid_farmer_01}
		local table_02 = {guid_master,guid_farmer_02}
		master_list[1] = table_01
		master_list[2] = table_02
		farmer_01_list[1] = table_01
		farmer_02_list[1] = table_02
	else
		local table_pair = {guid_farmer_01,guid_farmer_02}
		farmer_01_list[1] = table_pair
		farmer_02_list[1] = table_pair
	end
end
function can_pair_game(player01,player02)
	local guid_01 = player01.guid
	local guid_02 = player02.guid

	local guid_01_list = player01.friend_list or g_land_friend_list[guid_01]
	player01.friend_list = player01.friend_list or guid_01_list
	local guid_02_list = player02.friend_list or g_land_friend_list[guid_02]
	player02.friend_list = player02.friend_list or guid_02_list
	
	if guid_01_list then
		for k,v in pairs(guid_01_list) do
			for k1,v1 in pairs(v) do
				if v1 == guid_02 then return false end
			end
		end
	end
	if guid_02_list then
		for k,v in pairs(guid_02_list) do
			for k1,v1 in pairs(v) do
				if v1 == guid_01 then return false end
			end
		end
	end
	return true
...
function land_table:call_double(player,is_double)
	if self.first_turn == player.chair_id then
		return --地主不能加倍
	end
...
function  land_table:finishgame(player)
	...
	if self.status == LAND_STATUS_PLAYOFFLINE then
		...
		for i,v in ipairs(self.player_list_) do
			...
			if v.chair_id == offcharid then	
				...
				v:cost_money({{money_type = ITEM_PRICE_TYPE_GOLD, money = (m_f1_score + m_f2_score)--[[land_score *2]]}}, LOG_MONEY_OPT_TYPE_LAND)
			else
				s_type = 2
		...
		if self.flag_land == player.chair_id then
			...
			for i,v in ipairs(self.player_list_) do
				...
				if self.flag_land == v.chair_id then
					...
					notify.pb_conclude[v.chair_id].score = -farmer_score
					v:cost_money({{money_type = ITEM_PRICE_TYPE_GOLD, money = farmer_score}}, LOG_MONEY_OPT_TYPE_LAND)
			...
			for i,v in ipairs(self.player_list_) do
				...
				if self.flag_land == v.chair_id then
					...
					v:cost_money({{money_type = ITEM_PRICE_TYPE_GOLD, money = land_score}}, LOG_MONEY_OPT_TYPE_LAND)
					...
					if not self:isDroppedline(v) and not offlinePunishment_flag then
	...
	local f01_player = nil
	local f02_player = nil
	for i,v in ipairs(self.player_list_) do
		if v then
			if v.is_double then
	...
	register_friend_pair(master_player,f01_player,f02_player,land_master_win)
	...
	for i,v in pairs (self.player_list_) do
		if game_switch == 1 then--游戏将进入维护阶段
			if  v and v.is_player == true then 
				send2client_pb(v, "SC_StandUpAndExitRoom", {
...
function land_table:canEnter(player)
	...
	for _,v in ipairs(self.player_list_) do		
		if v and v.guid ~= player.guid then
			...
			if not can_pair_game(player,v) then
...
function land_table:tick()
	...
	--end
	if self.status == LAND_STATUS_FREE then
		if self.time0_ - get_second_time() < 2 then
			self.time0_ = get_second_time()
